...
{    
    ...
    void ValidateInputToCreateNewPlaylist(std::string user_input,char delimiter, std::string chars_expected);
    //std::pair<bool,string> ValidateUserSelectedTracksId(std::string); //todo
    //bool ValidateTrackIdIfExist(int);                                 //todo
    //bool SavePlayListIntoFile(std::string pls_name_and_tracks_ids);   //todo
...
void ValidateInputToCreateNewPlaylist(std::string user_input,char delimiter, std::string chars_expected)
{
    ...
    //clean packs (spaces - reject it there is something else then number)
    temp_string = GeneralFuncitons::cleanStringFromAllOtherSigns(user_input,chars_expected);

    //std::vector<int> general function (std::string user_input,char delimiter, std::string chars_expected)

    //split by delimiter
    std::vector<std::string>* v_ptr;
    v_ptr = GeneralFuncitons::SplitStringByDelimiter(temp_string,delimiter);

    //str-*v into int-v
    std::vector<int> v_int;
    for(std::string &v_ptr_el: *v_ptr ){v_int.push_back(std::stoi(v_ptr_el));}
    delete v_ptr;
    v_ptr = nullptr;

    //remove dubles - we cannot sort because user could use his own order
    


    //std::pair<std::vector<int>,std::vector<int>) playlist_i (std::vector<int> cleaned packages, std::vector<int> & id_s from list)
    //validate 

//remove spaces,
//check delimiter, 
//check if  delimited part is number
//compare numbers from list with user input
//return info what was rejected, what was accepted
//ask about  if result is correct - save it or modify 
}
...
std::pair<int,std::string> Playlist_I::CreateNewPlaylist(std::string path_to_pls_list_file)
{
        ...
        PrintPlaylistContent(this->pls_local_tracks_to_pick);


        std::string user_input;
        std::cout<<"\nPlease put in all track id's which you would like have at new playlist\n";
        std::cout<<"Track id should be separated by coma\n";
        std::cout<<"If you type 'all', all track will be added\n";
        std::cout<<"Saved order will be like you type in, but after creation you can change it\n";
        std::cout<<"Enter tracks id:";
        std::cin>>user_input;
        ...
        //return info what was rejected, what was accepted
        //ask about  if result is correct - save it or modify 
        /*ValidateInputToCreateNewPlaylist(user_input,Setting::SettingPlaylistFile::delimiter_used_in_all_file)
